...
public class DataPersistanceManager : MonoBehaviour
{
    ...
    public async void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        Debug.Log("Amount of objects is" + dataPersistanceobjects.Count);
        if (this.selectedSaveId != 0) { 
            await LoadGame(); 
        }
    }
    ...
    public async Task GoToNextLevel()
    {
        ...
        gameData.levelNum++;
        gameData.characterLives = 10;
        gameData.characterCoins =Character.Instance.Coins;
        await dataHandler.Save(gameData, currentUserId, selectedSaveId);
    }
